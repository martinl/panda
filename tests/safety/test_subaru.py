...
class TestSubaruCrosstrekHSafety(TestSubaruSafety):
  TX_MSGS = [[0x122, 0], [0x321, 0], [0x322, 0], [0x40, 2], [0x139, 2]]
  FWD_BLACKLISTED_ADDRS = {0: [0x40, 0x139], 2: [0x122, 0x321, 0x322]}
  ...
  def setUp(self):
    ...
    self.safety = libpandasafety_py.libpandasafety
    self.safety.set_safety_hooks(Panda.SAFETY_SUBARU_CROSSTREK_H, 0)
    self.safety.init_tests()
...
class TestSubaruForesterHSafety(TestSubaruSafety):

  def setUp(self):
    self.packer = CANPackerPanda("subaru_global_2017_generated")
    self.safety = libpandasafety_py.libpandasafety
    self.safety.set_safety_hooks(Panda.SAFETY_SUBARU_FORESTER_H, 0)
    self.safety.init_tests()
